...
const Home: React.FC = () => {
  ...
  const clearInput = () => {
    ...
    if (operators.includes(inp)) {
      ...
      if (lastNum) numArry.push(Number(lastNum));
  ...
  const calculateFunction = (numArray, operatorArray) => {
    ...
    for (i = 0; i < operatorArray.length; i++) {
      if (operatorArray[i] == "(") {
        let bracketOperatorArray = [],
          count = 0,
          k;

        for (k = i + 1; operatorArray[k] != ")"; k++) {
          if (!operatorArray[k]) break;
        ...
        let countNumbers = 0;
        if (count % 2 != 0) countNumbers = count * 2;
        else countNumbers = count * 2 - 1;
  ...
  return (
    <div className={styles.home}>
      <div className={styles.home__container}>
        <TextInput
          className={styles.home__operation}
          value={displayInput}
          disabled
        />
        <TextInput
          className={styles.home__result}
          value={displayNumber}
          disabled
        />
